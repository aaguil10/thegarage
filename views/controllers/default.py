...
def available():
    ...
    listfrd1 = db(db.link.friender == auth.user_id).select()
    listfrd2 = db(db.link.target == auth.user_id).select()
...
def Borrowed():
    ...
    items3 = db(db.items.item_state==2).select()
    return dict(items1=items1, items2=items2,items3=items3, )
    return dict()
